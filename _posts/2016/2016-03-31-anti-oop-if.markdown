...
제가 현업에서 업무를 진행할 시 팀원들에게 가장 많이 하는 말 중에 하나가 **가능한 분기문을 없애야하 한다.**
...
위와 같은 코드가 보이면 일반적으로 중복 제거를 위해서 **메소드 추출** 를 통한 리팩토링을 하게 됩니다.
...
기법입니다. 여기에서 중요한 것은 `getDiscountAmt` 즉 **할인에 관한 정책 분기를 굳이 결제 서비스
객체의 책임으로 두는게 맞느냐** 입니다. 아무리 봐도 할인과 결제는 분리되는 것이 낫습니다.
...
구현을 간단히 설명하면 각 실할인정책이 하나의 클래스로 분리된다고 보시면 됩니다. 이러한 리팩터링 기법을
**인터페이스 추출** 이라고 부릅니다.

    원래 클래스 추출 후 다형성이 요구될 경우 인터페이스 추출을 하지만 클래스 추출 과정은 생략 하였습니다.
...
몇가지의 생성 패턴 중 가장 대중적인 **추상 팩토리 패턴** 을 사용하겠습니다.
...
**`DIP`를 통해서 결합성이 큰 구상클래스가 아니라 유연한 인터페이스를 바탕으로 객체 의존성 확보**
...
사용하게 되니 두 패턴은 거의 같이 사용하게 된다고 봐도 무방하다.
...
이 정도 리팩터링 해도 객체지향적이다 라고 말할 수 있다. 만약 새로운 할인 정책이 추가되면 `PaymentService`와
...
**(클라이언트 코드) 변경(수정)에는 닫혀 있고, 확장(할인정책 추가)에는 열려 있다.**
...
- DDD with JPA 짱짱맨????
- **객체지향적 프로그래밍을 통해서 분기문을 없애는 노력을 기울이면 유연하고 응집력 있는 코드를 얻을 수 있다.**
